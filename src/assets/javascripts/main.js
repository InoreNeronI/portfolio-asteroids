...
let s = (_) => {
  ...
  document.querySelector('#play-start').addEventListener('click', () => {
    ...
    if (soundController.sound) soundController.soundInit();
    else soundController.soundMute();
  });
  ...
  _.draw = () => {
    ...
    if (cursor.life > 0) {
      ...
      if (_.frameCount % 20 === 0 && asteroidsController.asteroids.length) {
        explosionsController.createExplosion(asteroidsController.asteroids[0].pos, asteroidsController.asteroids[0].w);
      ...
      if (_.frameCount % 20 === 10 && lootsController.loots.length) {
        explosionsController.createExplosion(lootsController.loots[0].pos, lootsController.loots[0].w);
