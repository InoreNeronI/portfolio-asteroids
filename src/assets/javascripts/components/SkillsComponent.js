...
class App extends React.Component {
  ...
  handleClickSortGrid() {
    if (this.state.sort === 'grid') return;
    this.setState({
      loading: true,
    });
    let _ = this;
  ...
  handleClickSortList() {
    if (this.state.sort === 'list') return;
  ...
  render() {
    ...
    for (let [index, el] of elements.entries()) {
      ...
      if (el.el === 'Space' && this.state.sort === 'grid') {
        items.push(<Space key={index} value={el.value} />);
      }
    }
    return (
      <div className={this.state.loading ? 'loading' : ''}>
        <div className="sort">
          <div className={(this.state.sort == 'grid' ? 'active' : '') + ' grid'} onClick={this.handleClickSortGrid}>
            <div className="carret"></div>
            <div className="carret"></div>
            <div className="carret"></div>
            <div className="carret"></div>
          </div>
          <div className={(this.state.sort == 'list' ? 'active' : '') + ' list'} onClick={this.handleClickSortList}>
            <div className="tiret"></div>
            <div className="tiret"></div>
            <div className="tiret"></div>
          </div>
        </div>
        <ul className={this.state.sort}>{items}</ul>
      </div>
    );
  }
}
