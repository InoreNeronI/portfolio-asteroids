...
class AsteroidsController {
  ...
  asteroidCreate(size, origin, velocity) {
    if (typeof size === 'undefined') return console.error('asteroidCreate has size undefined');
    ...
    if (typeof origin !== 'undefined') {
      ...
      switch (borderSpawn) {
        case 0:
          ...
          break;
        case 1:
          position.set(this._.random(-this._.width / offsetDiv, 0), this._.random(0, this._.height));
          break;
        case 2:
          position.set(this._.random(0, this._.width), this._.random(-this._.height / offsetDiv, 0));
          break;
        case 3:
          position.set(
    ...
    this.asteroids.push(new Asteroid(this._, size, position, velocity));
  }

  asteroidRemove(index) {
    if (typeof index === 'undefined') return console.error('asteroidRemove has index undefined');
  ...
  asteroidsSpawn(levelController) {
    ...
    for (let a = 0; a < asteroidsLength; a++) {
      let randShape,
        rand = Math.floor(this._.random(1, 4));
